...
5. **Custom MDX components** live in `src/components/content/mdx/` and are wired into the `MDXComponents` map in `mdx/index.ts`. Notable overrides:
   ...
   - Interactive features register through `mdx/interactive.ts`. Compose the shared panel and choice controls in `src/components/content/interactive/`; keep feature data and behavior in its own content folder. See that folder's README for the addition steps.
