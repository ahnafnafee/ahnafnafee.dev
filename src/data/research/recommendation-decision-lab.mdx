...
Two diagnostics help explain the failure without reducing it to a single cause. The target was in the promotable shortlist for 33.21% of Musical Instruments requests, and unconstrained lexical search using earlier review prose retrieved it in 2.42% of requests with usable wording. Those rates have different denominators and cannot be multiplied into a measured joint ceiling. An exact target title retrieved its product in 99.98% of eligible cases, but that target-informed probe does not establish quality on natural queries. Published search phrases reached 47.72% on a selected set of 681 matched requests; their positive paired difference is a full-split average, not evidence of general benefit from real user wording. The [aggregate results and evaluation note](https://github.com/ahnafnafee/recommendation-decision-lab/blob/main/reports/phrase_to_products.md) show all six wording conditions.
