...
summary: 'How I built Rally, a gamified, crowdsourced map that shows which tennis, pickleball, and soccer courts are free right now, using an Expo and React Native app on Supabase Postgres with PostGIS spatial queries, GPS proof-of-presence check-ins, and server-side-only scoring so the crowdsourced map stays trustworthy.'
featured: true
...
<div className='my-6 flex flex-col items-center justify-center gap-4 sm:flex-row sm:items-start'>
  ...
  <img
    ...
    alt="Rally showing a tapped court's live card with its address, surface, lights, and free-or-busy status, so you know before you go"
    className='w-full max-w-xs rounded-xl border border-gray-200 shadow-sm dark:border-gray-700'
  />
</div>
...
<div className='my-6 flex flex-col items-center justify-center gap-4 sm:flex-row sm:items-start'>
  <img
    ...
    alt="Rally's pickup-game screen where every player checks in by GPS at the court, the game verifies attendance, and each player earns points"
    className='w-full max-w-xs rounded-xl border border-gray-200 shadow-sm dark:border-gray-700'
  />
  <img
    src='/images/rally-profile.png'
    alt="Rally's profile screen showing rank progress from Rookie to Scout, a live season tally of Aces, reputation, and streak, and a badge pin board"
    className='w-full max-w-xs rounded-xl border border-gray-200 shadow-sm dark:border-gray-700'
  />
</div>
...
<figure className='my-8'>
  <img
    ...
    alt='Animated Rally promo: the live court map, a court status card, a pickup game with GPS check-in, and the player profile with ranks and Aces'
    loading='lazy'
