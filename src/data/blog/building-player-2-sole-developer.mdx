...
Two pieces were more interesting than plumbing. The home feed is a two-stage retrieve-then-rank mixer modeled on the algorithm X open-sourced, except mine is transparent SQL and arithmetic sized for the userbase I actually have, with no black box in the middle. Every card it shows records whether you saw it, tapped it, or dismissed it, which quietly builds the labelled dataset a learned ranker would need later. Matchmaking follows the same spirit: weighted cosine similarity over survey answers, a binary similarity score over game libraries, and a diversity re-rank so you never get five near-identical suggestions in a row.
...
## Marketing Is Engineering Too

This is the part I did not expect to own, and it turned out to be as much work as any service. App store optimization is a real discipline, so I treated it like one. I wrote a strategy grounded in an actual competitor teardown, mapped the search terms worth ranking for, and settled on a clear positioning wedge: match on the games you own and the way you play, not just your rank. The store listings and release notes are versioned like source code and localized into nine languages. The screenshots are exported from a single Figma system through a documented, repeatable process instead of being hand-cropped fresh every release.

I also planned for how software gets found now. Beyond the two app stores, that means being the kind of thing an answer engine will cite, so the plan covers listicles, community threads, and structured data that assistants like ChatGPT and Perplexity can actually read. Marketing, approached this way, is just another system with inputs, versioning, and a feedback loop.

<div className='not-prose my-8 grid grid-cols-2 gap-3 sm:grid-cols-3'>
  <img
    src='/images/player2/player2-paywall.webp'
    alt='Player 2 PRO subscription paywall'
    loading='lazy'
    className='w-full rounded-xl border border-gray-200 dark:border-gray-800'
  />
  <img
    src='/images/player2/player2-store.webp'
    alt='Player 2 in-app cosmetics store'
    loading='lazy'
    className='w-full rounded-xl border border-gray-200 dark:border-gray-800'
  />
  <img
    src='/images/player2/player2-pcoins-store.webp'
    alt='Player 2 Player Coins purchase screen'
    loading='lazy'
    className='w-full rounded-xl border border-gray-200 dark:border-gray-800'
  />
</div>
